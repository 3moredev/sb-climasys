#!/usr/bin/env python3
"""
Import CSV Data with Fixed Column Casing
"""
...
def import_csv_data(conn, csv_file, table_name):
    ...
    try:
        ...
        # Create column mapping (exact case match)
        column_mapping = {}
        for csv_col in df.columns:
            ...
            if csv_col in table_columns:
                ...
                for table_col in table_columns:
                    if table_col.lower() == csv_col_lower:
        ...
        for csv_col in df.columns:
            if csv_col in column_mapping:
                table_col = column_mapping[csv_col]
        ...
        for col in mapped_df.columns:
            ...
            mapped_df[col] = mapped_df[col].where(pd.notnull(mapped_df[col]), None)
        ...
        insert_sql = f"""
            INSERT INTO climasys_dev.{table_name} ({', '.join([f'"{col}"' for col in columns])})
            VALUES ({placeholders})
...
def main():
    """Main function"""
    logger.info("🚀 Importing CSV Data with Fixed Column Casing")
    logger.info("=" * 70)
    ...
    # Save import results
    results_file = f'import_results_fixed_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'

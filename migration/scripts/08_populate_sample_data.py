#!/usr/bin/env python3
"""
Climasys Sample Data Population Script

This script populates the PostgreSQL tables with sample data for testing.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('sample_data_population.log'),
...
class SampleDataPopulator:
    def __init__(self, postgres_config):
        """Initialize the data populator with PostgreSQL configuration."""
    ...
    def populate_doctors(self):
        """Populate doctor_master table with sample data."""
        logger.info("Populating doctor_master table...")
        ...
        for doctor in doctors_data:
            self.cursor.execute(insert_query, doctor)
        
        logger.info(f"Inserted {len(doctors_data)} doctors")
    
    def populate_clinics(self):
        """Populate clinic_master table with sample data."""
        logger.info("Populating clinic_master table...")
        ...
        for clinic in clinics_data:
            self.cursor.execute(insert_query, clinic)
        
        logger.info(f"Inserted {len(clinics_data)} clinics")
    
    def populate_users(self):
        """Populate user_master table with sample data."""
        logger.info("Populating user_master table...")
    ...
    def populate_user_roles(self):
        """Populate user_role table with sample data."""
        logger.info("Populating user_role table...")
        
        # Get user IDs first
        self.cursor.execute("SELECT id, login_id FROM user_master ORDER BY id")
        ...
        for role in user_roles_data:
            self.cursor.execute(insert_query, role)
        
        logger.info(f"Inserted {len(user_roles_data)} user roles")
    
    def populate_patients(self):
        """Populate patient_master table with sample data."""
        logger.info("Populating patient_master table...")
        ...
        for patient in patients_data:
            self.cursor.execute(insert_query, patient)
        
        logger.info(f"Inserted {len(patients_data)} patients")
    
    def populate_patient_visits(self):
        """Populate patient_visits table with sample data."""
        logger.info("Populating patient_visits table...")
        ...
        for visit in visits_data:
            self.cursor.execute(insert_query, visit)
        
        logger.info(f"Inserted {len(visits_data)} patient visits")
    
    def populate_system_params(self):
        """Populate system_params table with sample data."""
        logger.info("Populating system_params table...")
        ...
        for param in system_params_data:
    ...
    def run_population(self):
        ...
        try:
            ...
            self.populate_clinics()
            self.populate_users()
            self.populate_user_roles()
            self.populate_patients()
            self.populate_patient_visits()
            self.populate_system_params()
...
def main():
    ...
    logger.info("Starting Climasys Sample Data Population")
    logger.info("=" * 50)
    
    # Create populator instance
    populator = SampleDataPopulator(postgres_config)

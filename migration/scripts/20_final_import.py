#!/usr/bin/env python3
"""
Final Data Import Script for Climasys

This script imports all data using the correct column mappings
from the actual SQL Server schema.
"""
...
class FinalImporter:
    ...
    def import_language_master(self, csv_file):
        """Import language master data with correct column mapping."""
        try:
            df = pd.read_csv(csv_file)
            if df.empty:
                return 0
            
            # The actual CSV has 'ID' column, map it to 'language_id'
            df = df.rename(columns={'ID': 'language_id'})
            
            # Add language_name based on language_id (you may need to adjust this)
            df['language_name'] = df['language_id'].astype(str)
            ...
            columns_str = ', '.join(columns)
            placeholders = ', '.join(['%s'] * len(columns))
            insert_query = f"INSERT INTO language_master ({columns_str}) VALUES ({placeholders})"
            
            data_tuples = [tuple(row) for row in df[columns].values]
            self.cursor.executemany(insert_query, data_tuples)
            
            logger.info(f"Imported {len(data_tuples)} language records")
            return len(data_tuples)
            
        except Exception as e:
            logger.error(f"Failed to import language master: {e}")
            return 0
    ...
    def import_all_data(self, data_dir="extracted_data"):
        ...
        try:
            ...
            import_operations = [
                ...
                ("Language_Master.csv", self.import_language_master),
            ...
            # Save import summary
            summary_file = os.path.join(data_dir, "final_import_summary.json")
            with open(summary_file, 'w') as f:
                json.dump(import_summary, f, indent=2)
            
            logger.info(f"Final import completed. Summary saved to {summary_file}")
...
def main():
    """Main function to run the final import."""
    ...
    logger.info("Starting Final PostgreSQL Data Import")
    logger.info("=" * 60)
    
    # Create importer instance
    importer = FinalImporter(postgres_config)
    ...
    try:
        ...
        logger.info("Importing data with correct column mappings...")
        ...
        logger.info("Final data import successful!")

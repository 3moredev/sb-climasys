#!/usr/bin/env python3
"""
Working Data Import Script for Climasys
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('working_import.log'),
...
class WorkingImporter:
    def __init__(self, postgres_config):
        """Initialize the working importer."""
    ...
    def import_all_data(self, data_dir="extracted_data"):
        ...
        try:
            ...
            # Save import summary
            summary_file = os.path.join(data_dir, "working_import_summary.json")
            with open(summary_file, 'w') as f:
                json.dump(import_summary, f, indent=2)
            
            logger.info(f"Working import completed. Summary saved to {summary_file}")
...
def main():
    """Main function to run the working import."""
    ...
    logger.info("Starting Working PostgreSQL Data Import")
    logger.info("=" * 60)
    
    # Create importer instance
    importer = WorkingImporter(postgres_config)
    ...
    try:
        ...
        logger.info("Working data import successful!")
        
    except Exception as e:
        logger.error(f"Working data import failed: {e}")
        sys.exit(1)
    finally:

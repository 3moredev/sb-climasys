...
public class VisitController {
    ...
    /**
     * Helper method to extract string value from Map with multiple possible key
     * names
     */
    ...
    /**
     * Helper method to extract integer value from Map with multiple possible key
     * names
     */
    ...
    private void convertTimezoneInRow(Map<String, Object> row) {
        try {
            ...
            System.out.println("DEBUG - Raw_Visit_Time: " + rawVisitTime + " (type: "
                    + (rawVisitTime != null ? rawVisitTime.getClass().getSimpleName() : "null") + ")");
            ...
            if (rawVisitTime != null) {
                ...
                if (timeStr.matches("\\d{2}:\\d{2}(:\\d{2})?")) {
                    ...
                    try {
                        ...
                        System.out.println("DEBUG - Converted UTC to " + timezoneUtils.getTimezoneDisplayName() + ": "
                        ...
                        System.out.println("ERROR - Timezone conversion failed, using original time: "
                                + timezoneException.getMessage());
            ...
            System.out.println("DEBUG - Online_Appointment_Time: " + onlineTime + " (type: "
                    + (onlineTime != null ? onlineTime.getClass().getSimpleName() : "null") + ")");
            ...
            if (onlineTime != null) {
                ...
                if (onlineTimeStr.matches("\\d{2}:\\d{2}(:\\d{2})?")) {
                    ...
                    try {
                        ...
                        String convertedOnlineTime = String.format("%02d:%02d", targetTime.getHour(),
                                targetTime.getMinute());
                        System.out.println(
                                "DEBUG - Converted online time UTC to " + timezoneUtils.getTimezoneDisplayName() + ": "
                                        + onlineTimeStr + " -> " + convertedOnlineTime);
                        ...
                        System.out.println("ERROR - Online time timezone conversion failed, using original time: "
                                + timezoneException.getMessage());
                        // Fallback to original time if timezone conversion fails
                        String fallbackTime = String.format("%02d:%02d", hours, minutes);
    ...
    public record ComprehensiveVisitDataRequest(
            ...
            // Treatment arrays (diagnosis, medicines, prescriptions, investigations,
            // complaints)
    ...
    public ResponseEntity<?> addToVisit(@RequestBody AddToVisitRequest req) {
        try {
            ...
            parameters.put("p_time_Visit_Time",
                    req.visitTime() != null ? java.sql.Time.valueOf(req.visitTime()) : null);
    ...
    /**
     * Comprehensive API to save all patient visit data matching the form fields
     * from screenshot
    ...
    /**
     * Comprehensive API to save all patient visit data using JPA (new
     * implementation)
     * This endpoint uses JPA instead of stored procedures for better
     * maintainability
     */
    ...
    public ResponseEntity<?> saveComprehensiveVisitDataJpa(@RequestBody ComprehensiveVisitDataRequest req) {
        try {
            logger.info(
                    "Received comprehensive-save-jpa request - PatientId: {}, DoctorId: {}, ClinicId: {}, ShiftId: {}, VisitNo: {}, StatusId: {}",
                    req.patientId(), req.doctorId(), req.clinicId(), req.shiftId(), req.patientVisitNo(),
                    req.statusId());
            logger.info(
                    "REFERRAL DEBUG: referBy={}, referralName={}, referralContact={}, referralEmail={}, referralAddress={}",
                    req.referBy(), req.referralName(), req.referralContact(), req.referralEmail(),
                    req.referralAddress());
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "Patient Visit Number must be a valid number: " + req.patientVisitNo());
            }
            ...
            if (req.statusId() != null && req.statusId() <= 0) {
                throw new IllegalArgumentException(
                        "Status ID must be greater than 0. Valid status IDs are 6 (Submit) or 9 (Save)");
            }
            ...
            VisitJpaService.ComprehensiveVisitRequest serviceRequest = new VisitJpaService.ComprehensiveVisitRequest(
                    ...
                    req.followUpType() != null && !req.followUpType().trim().isEmpty()
                            ? Short.parseShort(req.followUpType())
                            : null,
            ...
            try {
                ...
                if (visitDateObj != null) {
                    ...
                    logger.info("Using visit_date from saved result: {} (original parsed: {})", actualVisitDate,
                            visitDate);
                } else {
            ...
            logger.info("Final visitDate to use for diagnosis/medicine inserts: {} (Timestamp: {})", actualVisitDate,
                    visitDateTs);
            logger.info(
                    "Composite key values - patientId: {}, doctorId: {}, clinicId: {}, shiftId: {}, patientVisitNo: {}",
                    patientId, doctorId, clinicId, shiftIdVal, patientVisitNoVal);

            logger.info(
                    "Preparing to persist treatment arrays - complaintsRows: {}, diagnosisRows: {}, medicineRows: {}, prescriptionRows: {}, investigationRows: {}, dressingBodyParts: {}",
            ...
            if (result.get("success") != null && (Boolean) result.get("success")) {
                ...
                try {
                    ...
                    if (req.diagnosisRows() != null && !req.diagnosisRows().isEmpty()) {
                        ...
                        // CRITICAL: Query the EXACT visit_date from patient_visits to ensure foreign
                        // key matches
                        // This is essential because the saved visit might have a slightly different
                        // timestamp
                        ...
                        List<java.sql.Timestamp> exactVisitDates = jdbcTemplate.queryForList(getExactVisitDateSql,
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            ...
                            logger.info("Retrieved EXACT visit_date from patient_visits: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert diagnosis without parent record.");
                            result.put("diagnosisInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert diagnosis without matching patient_visits record");
                        }
                        ...
                        int deletedCount = jdbcTemplate.update(delDiag, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        for (Map<String, Object> row : req.diagnosisRows()) {
                            ...
                            if (!shortDesc.isEmpty() || !diagnosis.isEmpty()) {
                                try {
                                    logger.info("Attempting INSERT with exact visit_date: {} for diagnosis: {}",
                                            exactVisitDateTs, diagnosis);
                                    int rowsAffected = jdbcTemplate.update(insDiag, patientId, exactVisitDateTs,
                                            patientVisitNoVal, shiftIdVal, clinicId, doctorId, shortDesc, diagnosis,
                                            false, now, userIdVal, now, userIdVal);
                                    if (rowsAffected > 0) {
                                        insertedCount++;
                                        logger.info(
                                                "✓ Successfully inserted diagnosis row {}: short_description='{}', desease_description='{}', visitDate={}",
                                                insertedCount, shortDesc, diagnosis, exactVisitDateTs);
                                    } else {
                                        logger.error(
                                                "✗ INSERT statement returned 0 rows affected for diagnosis: short_description='{}', desease_description='{}'",
                                                shortDesc, diagnosis);
                                        result.put("diagnosisInsertWarning",
                                                "Some diagnosis rows may not have been inserted");
                                    }
                                } catch (Exception insertEx) {
                                    logger.error(
                                            "✗ Error inserting diagnosis row: short_description='{}', desease_description='{}', error: {}",
                                            shortDesc, diagnosis, insertEx.getMessage(), insertEx);
                                    logger.error(
                                            "✗ Failed INSERT parameters - patientId: {}, visitDate: {}, patientVisitNo: {}, shiftId: {}, clinicId: {}, doctorId: {}",
                                            patientId, exactVisitDateTs, patientVisitNoVal, shiftIdVal, clinicId,
                                            doctorId);
                        ...
                        logger.info("Successfully inserted {} diagnosis records for patient: {}, visit: {}",
                        ...
                        if (insertedCount > 0) {
                            ...
                            Integer actualCount = jdbcTemplate.queryForObject(verifySql, Integer.class, patientId,
                                    doctorId, clinicId, shiftIdVal, patientVisitNoVal, exactVisitDateTs);
                            logger.info(
                                    "✓ Verification: Found {} diagnosis records in database after insert (expected: {}, visitDate: {})",
                                    actualCount, insertedCount, exactVisitDateTs);
                            if (actualCount == null || actualCount == 0) {
                                logger.error(
                                        "✗ CRITICAL: No diagnosis records found in database after insert attempt! visitDate used: {}",
                                        exactVisitDateTs);
                                result.put("diagnosisInsertWarning",
                                        "Diagnosis rows were not saved. Check logs for details.");
                                result.put("diagnosisInsertedCount", 0);
                            } else {
                                result.put("diagnosisInsertedCount", actualCount);
                                logger.info("✓ Confirmed: {} diagnosis records are now in visit_diagnosis table",
                                        actualCount);
                            }
                        }
                    } else {
                    ...
                    if (req.complaintsRows() != null && !req.complaintsRows().isEmpty()) {
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            exactVisitDateTs = exactVisitDates.get(0);
                            logger.info(
                                    "Retrieved EXACT visit_date from patient_visits for complaints: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert complaints without parent record.");
                            result.put("complaintsInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert complaints without matching patient_visits record");
                        }

                        // Delete existing complaint records for this visit (using exact visit_date to
                        // match foreign key)
                        ...
                        int deletedCount = jdbcTemplate.update(delComplaints, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        for (Map<String, Object> row : req.complaintsRows()) {
                            ...
                            logger.debug(
                                    ...
                                    shortDesc, complaintDesc, complaintComment);

                            // Insert if at least short_description has content (required field)
                            if (!shortDesc.isEmpty()) {
                                try {
                                    logger.info(
                                            "Attempting INSERT with exact visit_date: {} for complaint: short_description='{}', complaint_description='{}'",
                                            exactVisitDateTs, shortDesc, complaintDesc);
                                    int rowsAffected = jdbcTemplate.update(insComplaints, patientId, exactVisitDateTs,
                                            patientVisitNoVal, shiftIdVal, clinicId, doctorId, shortDesc, complaintDesc,
                                            complaintComment, false, now, userIdVal, now, userIdVal);
                                    if (rowsAffected > 0) {
                                        insertedCount++;
                                        logger.info(
                                                "✓ Successfully inserted complaint row {}: short_description='{}', complaint_description='{}', visitDate={}",
                                                insertedCount, shortDesc, complaintDesc, exactVisitDateTs);
                                    } else {
                                        logger.error(
                                                "✗ INSERT statement returned 0 rows affected for complaint: short_description='{}', complaint_description='{}'",
                                                shortDesc, complaintDesc);
                                        result.put("complaintsInsertWarning",
                                                "Some complaint rows may not have been inserted");
                                    }
                                } catch (Exception insertEx) {
                                    logger.error(
                                            "✗ Error inserting complaint row: short_description='{}', complaint_description='{}', error: {}",
                        ...
                        logger.info("Successfully inserted {} complaint records for patient: {}, visit: {}",
                                insertedCount, patientId, patientVisitNoVal);
                        ...
                        if (insertedCount > 0) {
                            ...
                            Integer actualCount = jdbcTemplate.queryForObject(verifySql, Integer.class, patientId,
                                    doctorId, clinicId, shiftIdVal, patientVisitNoVal, exactVisitDateTs);
                            logger.info(
                                    "✓ Verification: Found {} complaint records in database after insert (expected: {}, visitDate: {})",
                                    actualCount, insertedCount, exactVisitDateTs);
                            if (actualCount == null || actualCount == 0) {
                                logger.error(
                                        "✗ CRITICAL: No complaint records found in database after insert attempt! visitDate used: {}",
                                        exactVisitDateTs);
                                result.put("complaintsInsertWarning",
                                        "Complaint rows were not saved. Check logs for details.");
                                result.put("complaintsInsertedCount", 0);
                            } else {
                                result.put("complaintsInsertedCount", actualCount);
                                logger.info("✓ Confirmed: {} complaint records are now in visit_complaints table",
                                        actualCount);
                            }
                        }
                    } else {
                    ...
                    if (req.medicineRows() != null && !req.medicineRows().isEmpty()) {
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            exactVisitDateTs = exactVisitDates.get(0);
                            logger.info(
                                    "Retrieved EXACT visit_date from patient_visits for medicines: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert medicines without parent record.");
                            result.put("medicineInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert medicines without matching patient_visits record");
                        }
                        ...
                        int deletedCount = jdbcTemplate.update(delMed, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        for (Map<String, Object> row : req.medicineRows()) {
                            ...
                            if ((shortDesc != null && !shortDesc.isEmpty())
                                ...
                                try {
                                    logger.info("Attempting INSERT with exact visit_date: {} for medicine: {}",
                                            exactVisitDateTs, medicine);
                                    int rowsAffected = jdbcTemplate.update(insMed, patientId, exactVisitDateTs,
                                            patientVisitNoVal, shiftIdVal, clinicId, doctorId,
                                            shortDesc, medicine, morning, afternoon, night, noOfDays, instruction,
                                            false, now, userIdVal, now, userIdVal);
                                    if (rowsAffected > 0) {
                                        insertedCount++;
                                        logger.info(
                                                "✓ Successfully inserted medicine row {}: short_description='{}', medicine_description='{}', visitDate={}",
                                                insertedCount, shortDesc, medicine, exactVisitDateTs);
                                    } else {
                                        logger.error(
                                                "✗ INSERT statement returned 0 rows affected for medicine: short_description='{}', medicine_description='{}'",
                                                shortDesc, medicine);
                                        result.put("medicineInsertWarning",
                                                "Some medicine rows may not have been inserted");
                                    }
                                } catch (Exception insertEx) {
                                    logger.error(
                                            "✗ Error inserting medicine row: short_description='{}', medicine_description='{}', error: {}",
                        ...
                        logger.info("Successfully inserted {} medicine records for patient: {}, visit: {}",
                                insertedCount, patientId, patientVisitNoVal);
                    ...
                    if (req.prescriptionRows() != null && !req.prescriptionRows().isEmpty()) {
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            exactVisitDateTs = exactVisitDates.get(0);
                            logger.info(
                                    "Retrieved EXACT visit_date from patient_visits for prescriptions: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert prescriptions without parent record.");
                            result.put("prescriptionInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert prescriptions without matching patient_visits record");
                        }
                        ...
                        int deletedCount = jdbcTemplate.update(delPres, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        try {
                            ...
                            List<Integer> maxSeq = jdbcTemplate.queryForList(maxSeqSql, Integer.class, patientId,
                                    doctorId, clinicId, shiftIdVal, patientVisitNoVal, exactVisitDateTs);
                            if (maxSeq != null && !maxSeq.isEmpty() && maxSeq.get(0) != null) {
                                nextSequenceId = maxSeq.get(0);
                            }
                        } catch (Exception e) {
                            logger.warn("Failed to get next sequence ID for prescriptions, using default: {}",
                                    e.getMessage());
                        }
                        ...
                        for (Map<String, Object> row : req.prescriptionRows()) {
                            ...
                            // Parse prescription: expected format "Brand Name (Generic Name)" or just the
                            // name
                            ...
                            if ((brandName != null && !brandName.isEmpty())
                                ...
                                try {
                                    logger.info("Attempting INSERT with exact visit_date: {} for prescription: {}",
                                            exactVisitDateTs, prescription);
                                    int rowsAffected = jdbcTemplate.update(insPres, patientId, exactVisitDateTs,
                                            patientVisitNoVal, shiftIdVal, clinicId, doctorId,
                                            brandName, medicineName, "", "", "", morning, afternoon, night, noOfDays,
                                            instruction,
                                            nextSequenceId++, now, userIdVal, now, userIdVal, false);
                                    if (rowsAffected > 0) {
                                        insertedCount++;
                                        logger.info(
                                                "✓ Successfully inserted prescription row {}: brand_name='{}', medicine_name='{}', visitDate={}",
                                                insertedCount, brandName, medicineName, exactVisitDateTs);
                                    } else {
                                        logger.error(
                                                "✗ INSERT statement returned 0 rows affected for prescription: brand_name='{}', medicine_name='{}'",
                                                brandName, medicineName);
                                        result.put("prescriptionInsertWarning",
                                                "Some prescription rows may not have been inserted");
                                    }
                                } catch (Exception insertEx) {
                                    logger.error(
                                            "✗ Error inserting prescription row: brand_name='{}', medicine_name='{}', error: {}",
                        ...
                        logger.info("Successfully inserted {} prescription records for patient: {}, visit: {}",
                                insertedCount, patientId, patientVisitNoVal);
                    ...
                    if (req.investigationRows() != null && !req.investigationRows().isEmpty()) {
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            exactVisitDateTs = exactVisitDates.get(0);
                            logger.info(
                                    "Retrieved EXACT visit_date from patient_visits for investigations: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert investigations without parent record.");
                            result.put("investigationInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert investigations without matching patient_visits record");
                        }

                        // Delete existing investigation records for this visit (using exact visit_date
                        // to match foreign key)
                        ...
                        int deletedCount = jdbcTemplate.update(delInv, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        for (Map<String, Object> row : req.investigationRows()) {
                            ...
                            if (investigation != null && !investigation.isEmpty() && !investigation.equals("null")) {
                                ...
                                try {
                                    logger.info("Attempting INSERT with exact visit_date: {} for investigation: {}",
                                            exactVisitDateTs, investigation);
                                    int rowsAffected = jdbcTemplate.update(insInv, patientId, exactVisitDateTs,
                                            patientVisitNoVal, shiftIdVal, clinicId, doctorId,
                                            investigation, false);
                                    if (rowsAffected > 0) {
                                        insertedCount++;
                                        logger.info(
                                                "✓ Successfully inserted investigation row {}: lab_test_description='{}', visitDate={}",
                                                insertedCount, investigation, exactVisitDateTs);
                                    } else {
                                        logger.error(
                                                "✗ INSERT statement returned 0 rows affected for investigation: lab_test_description='{}'",
                                                investigation);
                                        result.put("investigationInsertWarning",
                                                "Some investigation rows may not have been inserted");
                                    }
                                } catch (Exception insertEx) {
                                    ...
                                    if (errorMsg != null && errorMsg.contains("duplicate key")) {
                                        logger.warn(
                                                "Duplicate key violation for investigation '{}' - may already exist in database, skipping",
                                                investigation);
                                        // Don't fail the entire request, just log and continue
                                    } else if (errorMsg != null && errorMsg.contains("foreign key constraint")) {
                                        logger.error(
                                                "✗ Foreign key constraint violation for investigation: '{}' - visit may not exist with exact visit_date: {}",
                                                investigation, exactVisitDateTs);
                                        logger.error(
                                                "✗ Failed INSERT parameters - patientId: {}, visitDate: {}, patientVisitNo: {}, shiftId: {}, clinicId: {}, doctorId: {}",
                                                patientId, exactVisitDateTs, patientVisitNoVal, shiftIdVal, clinicId,
                                                doctorId);
                                        result.put("investigationInsertError",
                                                "Foreign key constraint violation - visit may not exist");
                                        throw insertEx; // Re-throw foreign key errors as they indicate a serious issue
                                    } else {
                                        logger.error(
                                                "✗ Error inserting investigation row: lab_test_description='{}', error: {}",
                    ...
                    if (req.dressingBodyParts() != null && !req.dressingBodyParts().trim().isEmpty()) {
                        ...
                        logger.info(
                                "Processing dressing (body parts) textbox data for patient: {}, visit: {}, content length: {}",
                        ...
                        if (exactVisitDates != null && !exactVisitDates.isEmpty() && exactVisitDates.get(0) != null) {
                            exactVisitDateTs = exactVisitDates.get(0);
                            logger.info(
                                    "Retrieved EXACT visit_date from patient_visits for dressings: {} (was using: {})",
                                    exactVisitDateTs, visitDateTs);
                        } else {
                            logger.error(
                                    "CRITICAL: Could not find saved visit in patient_visits! Cannot insert dressings without parent record.");
                            result.put("dressingInsertError", "Parent visit not found in patient_visits table");
                            throw new IllegalStateException(
                                    "Parent visit not found: Cannot insert dressings without matching patient_visits record");
                        }

                        // Delete existing dressing records for this visit (matching stored procedure
                        // logic)
                        // The stored procedure deletes all dressings for the visit before inserting new
                        // ones
                        ...
                        int deletedCount = jdbcTemplate.update(delDressing, patientId, doctorId, clinicId, shiftIdVal,
                                patientVisitNoVal, exactVisitDateTs);
                        ...
                        try {
                            logger.info("Attempting INSERT with exact visit_date: {} for dressing (body parts): {}",
                                    exactVisitDateTs, dressingBodyParts);
                            int rowsAffected = jdbcTemplate.update(insDressing, patientId, exactVisitDateTs,
                                    patientVisitNoVal, shiftIdVal, clinicId, doctorId,
                                    dressingBodyParts, false, now, userIdVal, now, userIdVal);
                            if (rowsAffected > 0) {
                                logger.info(
                                        "✓ Successfully inserted dressing (body parts): dressing_description='{}', visitDate={}",
                                ...
                                Integer actualCount = jdbcTemplate.queryForObject(verifySql, Integer.class, patientId,
                                        doctorId, clinicId, shiftIdVal, patientVisitNoVal, exactVisitDateTs);
                                logger.info(
                                        "✓ Verification: Found {} dressing record(s) in database after insert (visitDate: {})",
                                        actualCount, exactVisitDateTs);
                                if (actualCount == null || actualCount == 0) {
                                    logger.error(
                                            "✗ CRITICAL: No dressing record found in database after insert attempt! visitDate used: {}",
                                            exactVisitDateTs);
                                    result.put("dressingInsertWarning",
                                            "Dressing (body parts) was not saved. Check logs for details.");
                                ...
                                logger.error(
                                        "✗ INSERT statement returned 0 rows affected for dressing (body parts): '{}'",
                                        dressingBodyParts);
                            ...
                            if (errorMsg != null && errorMsg.contains("duplicate key")) {
                                logger.warn(
                                        "Duplicate key violation for dressing (body parts) '{}' - may already exist in database",
                                        dressingBodyParts);
                                result.put("dressingInsertWarning", "Dressing may already exist");
                            } else if (errorMsg != null && errorMsg.contains("foreign key constraint")) {
                                logger.error(
                                        "✗ Foreign key constraint violation for dressing (body parts): '{}' - visit may not exist with exact visit_date: {}",
                                        dressingBodyParts, exactVisitDateTs);
                                logger.error(
                                        "✗ Failed INSERT parameters - patientId: {}, visitDate: {}, patientVisitNo: {}, shiftId: {}, clinicId: {}, doctorId: {}",
                                        patientId, exactVisitDateTs, patientVisitNoVal, shiftIdVal, clinicId, doctorId);
                                result.put("dressingInsertError",
                                        "Foreign key constraint violation - visit may not exist");
                                throw insertEx; // Re-throw foreign key errors as they indicate a serious issue
                            } else {
                    ...
                    if (req.instructionGroups() != null && !req.instructionGroups().isEmpty()) {
                        ...
                        try {
                            // CRITICAL: Use the EXACT visit_date from the saved visit entity (from result
                            // map)
                            // This ensures the visit_date used matches exactly what was saved in
                            // patient_visits table
                            // The saved entity's visitDate is the authoritative source - no need to query
                            // again
                            ...
                            if (visitDateFromResult != null) {
                                if (visitDateFromResult instanceof LocalDateTime) {
                                    ...
                                    logger.info("Using visit_date from saved result: {}",
                                            exactVisitDateForInstructions);
                                } else if (visitDateFromResult instanceof java.sql.Timestamp) {
                                    exactVisitDateForInstructions = ((java.sql.Timestamp) visitDateFromResult)
                                            .toLocalDateTime();
                                    useResultVisitDate = true;
                                    logger.info("Using visit_date from saved result (converted from Timestamp): {}",
                                            exactVisitDateForInstructions);
                                }
                            }

                            // Fallback: Query the database if result doesn't have visitDate (shouldn't
                            // happen, but safety check)
                            if (!useResultVisitDate) {
                                String getExactVisitDateSql = "SELECT visit_date FROM patient_visits WHERE patient_id = ? AND doctor_id = ? AND clinic_id = ? AND shift_id = ? AND patient_visit_no = ? AND delete_flag = false ORDER BY visit_date DESC LIMIT 1";
                                List<java.sql.Timestamp> exactVisitDates = jdbcTemplate.queryForList(
                                        getExactVisitDateSql, java.sql.Timestamp.class,
                                        patientId, doctorId, clinicId, shiftIdVal, patientVisitNoVal);

                                if (exactVisitDates != null && !exactVisitDates.isEmpty()
                                        && exactVisitDates.get(0) != null) {
                                    ...
                                    logger.info(
                                            "Retrieved EXACT visit_date from patient_visits query: {} (Timestamp: {})",
                                            exactVisitDateForInstructions, timestamp);
                                } else {
                                    logger.error(
                                            "CRITICAL: Could not find saved visit in patient_visits! Cannot insert instruction groups without parent record.");
                                    result.put("instructionGroupsInsertError",
                                            "Parent visit not found in patient_visits table");
                                    throw new IllegalStateException(
                                            "Parent visit not found: Cannot insert instruction groups without matching patient_visits record");
                                }
                            }

                            logger.info("Final visit_date to use for instruction groups: {}",
                                    exactVisitDateForInstructions);

                            // NOTE: We don't need to verify the visit exists because we just saved it in
                            // the same transaction
                            // The visit is guaranteed to exist since saveComprehensiveVisit succeeded
                            // Verification was causing issues because JPA entities might not be visible to
                            // JDBC queries
                            // until flush, but we can trust the visitDate from the saved entity
                            logger.info(
                                    "✓ Using visit_date from saved visit entity (no verification needed - visit was just saved)");

                            // Delete existing instruction groups for this visit (matching stored procedure
                            // logic)
                            visitGroupsInstructionsRepository.deleteByVisit(
                                    doctorId, clinicId, shiftIdVal, patientId, patientVisitNoVal,
                                    exactVisitDateForInstructions);
                            logger.info("Deleted existing instruction groups for visit using exact visit_date: {}",
                                    exactVisitDateForInstructions);
                            ...
                            for (Map<String, Object> instructionItem : req.instructionGroups()) {
                                String groupDescription = getStringValue(instructionItem, "groupDescription",
                                        "group_description");
                                String instructionsDescription = getStringValue(instructionItem,
                                        "instructionsDescription", "instructions_description");
                                Integer sequenceNo = getIntegerValue(instructionItem, "sequenceNo", "sequence_no");

                                logger.info(
                                        "Processing instruction item {} - groupDescription: '{}', instructionsDescription: '{}', sequenceNo: {}, fullItem: {}",
                                        visitInstructions.size() + 1, groupDescription, instructionsDescription,
                                        sequenceNo, instructionItem);

                                // Allow saving if at least groupDescription OR instructionsDescription is
                                // present
                                // Both fields are required in the table, but we'll use empty string if one is
                                // missing
                                boolean hasGroupDesc = groupDescription != null && !groupDescription.trim().isEmpty();
                                boolean hasInstrDesc = instructionsDescription != null
                                        && !instructionsDescription.trim().isEmpty();
                                ...
                                if (hasGroupDesc || hasInstrDesc) {
                                    ...
                                    if (!hasGroupDesc) {
                                        logger.warn(
                                                "⚠️ Instruction item missing groupDescription, using empty string. Item: {}",
                                                instructionItem);
                                    }
                                    if (!hasInstrDesc) {
                                        logger.warn(
                                                "⚠️ Instruction item missing instructionsDescription, using empty string. Item: {}",
                                                instructionItem);
                                    }
                                    ...
                                    logger.info("✓ Added instruction item to save list: group='{}', instruction='{}'",
                                            finalGroupDesc, finalInstrDesc);
                                } else {
                                    logger.warn(
                                            "⚠️ Skipping instruction group item - both groupDescription and instructionsDescription are missing/empty. Full item: {}",
                                            instructionItem);
                                }
                            }
                            ...
                            if (!visitInstructions.isEmpty()) {
                                ...
                                for (int i = 0; i < visitInstructions.size(); i++) {
                                    ...
                                    logger.info(
                                            "Instruction {}: doctorId={}, clinicId={}, shiftId={}, patientId={}, visitNo={}, visitDate={}, group={}, instruction={}",
                                            i + 1, vi.getDoctorId(), vi.getClinicId(), vi.getShiftId(),
                                            vi.getPatientId(),
                                            vi.getPatientVisitNo(), vi.getVisitDate(), vi.getGroupDescription(),
                                            vi.getInstructionsDescription());
                                }

                                // Save all instruction groups
                                List<VisitGroupsInstructions> savedInstructions = visitGroupsInstructionsRepository
                                        .saveAll(visitInstructions);
                                ...
                                long countAfterSave = visitGroupsInstructionsRepository
                                        .countByDoctorIdAndClinicIdAndShiftIdAndPatientIdAndPatientVisitNoAndVisitDate(
                                                doctorId, clinicId, shiftIdVal, patientId, patientVisitNoVal,
                                                exactVisitDateForInstructions);
                                logger.info(
                                        "✓ Verified: {} instruction group records now exist in database for this visit",
                                        countAfterSave);
                                ...
                                logger.warn(
                                        "No valid instruction group items to save after filtering. Total items received: {}",
                    ...
                    logger.error(
                            "Failed to persist treatment arrays (diagnosis/medicines/prescriptions/investigations/dressings/instructionGroups) for patient: {}, visit: {}",
                    ...
                    if (persistEx.getMessage() != null && persistEx.getMessage().contains("foreign key constraint")) {
                        logger.error(
                                "FOREIGN KEY CONSTRAINT VIOLATION - The visit might not exist or visitDate doesn't match exactly");
                        result.put("foreignKeyError", true);
                        result.put("foreignKeyDetails",
                                "Visit record might not exist with exact composite key. visitDate used: "
                                        + visitDateTs);
                    }
    ...
    private List<Map<String, Object>> getComprehensiveVisitData(String patientId, String clinicId,
        ...
        // SIMPLIFIED: Parameters: language_id (for GT join), patient_id, clinic_id,
        // patient_visit_no
    ...
    private String buildMainQuery(boolean includePaymentMaster) {
         ...
         * if (includePaymentMaster) {
         * query.
         * append("INNER JOIN payment_type_master PTM ON PTM.id = PV.payment_by_id ");
         * }
         * 
         * query.
         * append("LEFT JOIN patient_receipts rs ON PV.receipt_number = rs.receipt_number "
         * )
         * .append("WHERE PV.patient_id = ? ")
         * .append("AND PV.clinic_id = ? ")
         * .append("AND PV.patient_visit_no = ? ")
    ...
    /**
     * Get today's appointments for given date matching
     * USP_Get_TodaysAppointments_ForGivenDate stored procedure
     * 
    ...
    public ResponseEntity<?> getTodaysAppointmentsForGivenDate(
        ...
        try {
                ...
                if (timeRow.get("time_part") != null) {
                    ...
                    try {
                        ...
                        if (parts.length >= 2) {
                            ...
                            java.time.LocalTime utcTime = java.time.LocalTime.of(hours, minutes);
                            java.time.LocalTime targetTime = timezoneUtils.convertUtcToTargetTimezone(utcTime);
                            System.out.println("    UTC " + utcTime + " -> " + timezoneUtils.getTimezoneDisplayName()
                                    + " " + targetTime);
                        }
                    } catch (Exception e) {
            ...
            List<Map<String, Object>> resultSet1 = getSpecificDateAppointments(doctorId, clinicId, queryDate,
                    languageId);
    ...
    private List<Map<String, Object>> getSpecificDateAppointments(String doctorId, String clinicId,
        ...
        try {
            ...
            List<Map<String, Object>> result = jdbcTemplate.queryForList(query, languageId, doctorId, timezoneId,
                    futureDate);
    ...
    private List<Map<String, Object>> getTodayAndFutureAppointments(String doctorId, String clinicId,
            Integer languageId) {
        try {
            String query = "SELECT " +
                    "TO_CHAR(PV.visit_time::time, 'HH24:MI') AS Visit_Time, " +
    ...
    private List<Map<String, Object>> testBasicDataAvailability(String doctorId, String clinicId,
            java.sql.Date queryDate) {
     ...
     * USP_Insert_MedicineDataOverwrite stored procedure
     ...
     * This endpoint replicates the functionality of USP_Update_Addendum stored
     * procedure
     * 
    ...
    public ResponseEntity<?> updateAddendum(@RequestBody UpdateAddendumRequest request) {
        try {
            ...
            // Create service request
            VisitJpaService.UpdateAddendumRequest serviceRequest = new VisitJpaService.UpdateAddendumRequest(
                    request.addendum(),
                    request.visitDate(),
                    request.patientId(),
                    request.patientVisitNo(),

...
public interface AppointmentRepository extends JpaRepository<PatientVisit, Long> {
       ...
       Optional<PatientVisit> findByPatientIdAndPatientVisitNo(String patientId, Integer patientVisitNo);

       Optional<PatientVisit> findByPatientIdAndPatientVisitNoAndClinicIdAndShiftId(String patientId,
                     Integer patientVisitNo, String clinicId, Short shiftId);
       ...
       List<PatientVisit> findByDoctorIdAndVisitDateAfter(@Param("doctorId") String doctorId,
                     @Param("date") LocalDateTime date);

       // Find appointments for a doctor on a specific date
       @Query("SELECT pv FROM PatientVisit pv WHERE pv.doctorId = :doctorId AND pv.visitDate = :date")
       List<PatientVisit> findByDoctorIdAndVisitDate(@Param("doctorId") String doctorId,
                     @Param("date") LocalDateTime date);
       ...
       List<PatientVisit> findByDoctorIdAndVisitDateBetween(@Param("doctorId") String doctorId,
       ...
       // Check for appointment conflicts - same patient, same doctor, same date/shift,
       // specific statuses
       ...
       // Find appointments by patient, doctor, and exact datetime (since DB stores
       // both date and time in UTC)
       ...
       // Soft delete appointment by date and time (matching separate date and time
       // fields)
       @Modifying
       @Transactional
       ...
       @Modifying
       @Transactional
       @Query("UPDATE PatientVisit pv SET pv.onlineAppointmentTime = :onlineAppointmentTime, pv.doctorId = :doctorId, pv.statusId = :statusId, pv.modifiedOn = :modifiedOn, pv.modifiedbyName = :modifiedBy WHERE pv.patientId = :patientId AND pv.patientVisitNo = :patientVisitNo AND pv.shiftId = :shiftId AND pv.clinicId = :clinicId")

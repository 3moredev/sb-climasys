...
public interface CityTranslationRepository extends JpaRepository<CityTranslation, CityTranslationId> {
    ...
    List<CityTranslation> searchCitiesByLanguageAndName(@Param("searchStr") String searchStr,
            @Param("languageId") Integer languageId);

    @Query("SELECT ct FROM CityTranslation ct WHERE ct.id.stateId = :stateId")
    List<CityTranslation> findByStateId(@Param("stateId") String stateId);

    List<CityTranslation> findByIdCityIdInAndIdLanguageId(List<String> cityIds, Integer languageId);

    @Query("SELECT ct FROM CityTranslation ct ORDER BY ct.cityName")
    List<CityTranslation> findAllOrdered();

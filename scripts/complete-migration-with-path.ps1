...
if (Test-Path $PSQL_PATH) {
    ...
    Write-Host "[ERROR] PostgreSQL client (psql) not found at: $PSQL_PATH" -ForegroundColor Red
    Write-Host "[ERROR] Please check your PostgreSQL installation." -ForegroundColor Red
...
try {
    & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d postgres -c "CREATE DATABASE $DB_NAME;" 2>$null
...
try {
    & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_NAME -c "SELECT 1;" 2>$null
...
try {
    & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_NAME -f "$MIGRATION_DIR\01_create_schema.sql" 2>$null
...
try {
    & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_NAME -f "$MIGRATION_DIR\02_insert_reference_data.sql" 2>$null
...
try {
    & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_NAME -f "$MIGRATION_DIR\09_migrate_stored_procedures.sql" 2>$null
...
try {
    $tableCount = & $PSQL_PATH -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_NAME -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'climasys_dev';" 2>$null
